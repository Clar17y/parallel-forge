...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "(state = 'PAUSED' AND suspended_state IS NOT NULL AND suspended_state IN ('CREATED','PLANNING','AWAITING_PLAN_APPROVAL','PREPARING_WORKTREE','IMPLEMENTING','VALIDATING','REVIEWING','REMEDIATING','AWAITING_PR_APPROVAL','PUBLISHING_PR','MONITORING_PR','AWAITING_HUMAN_INTERVENTION','AWAITING_MERGE_APPROVAL','MERGING') AND suspension_kind IS NOT NULL AND suspension_kind = 'PAUSE' AND suspension_context IS NOT NULL AND suspension_context_schema_version IS NOT NULL AND suspension_context_schema_version >= 1) OR (state = 'AWAITING_HUMAN_INTERVENTION' AND suspended_state IS NOT NULL AND suspended_state IN ('PLANNING','PREPARING_WORKTREE','IMPLEMENTING','VALIDATING','REVIEWING','REMEDIATING','PUBLISHING_PR','MONITORING_PR','AWAITING_MERGE_APPROVAL','MERGING') AND suspension_kind IS NOT NULL AND suspension_kind = 'INTERVENTION' AND suspension_context IS NULL AND suspension_context_schema_version IS NULL) OR (state NOT IN ('PAUSED','AWAITING_HUMAN_INTERVENTION') AND suspended_state IS NULL AND suspension_kind IS NULL AND suspension_context IS NULL AND suspension_context_schema_version IS NULL)",
