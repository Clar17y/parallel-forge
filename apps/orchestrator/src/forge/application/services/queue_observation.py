...
class QueueObservationService:
    ...
    async def execute(self, command: CommandEnvelope, work: UnitOfWork) -> None:
        if (
            command.command_type != "observe_merge_queue"
            or command.status is not CommandStatus.LEASED
            or command.payload_schema_version != 1
            or set(command.payload) - RESUME_FIELDS != _FIELDS
            or type(command.payload["poll"]) is not int
            or command.payload["poll"] < 1
        ):
        ...
        settled = [
            e
            for e in events
            if e.event_type == "run.merge_queue_poll_settled"
        ...
        if pull is not None and pull.merged:
            merged_intent = await work.operations.begin(
                ...
                execution_owner=f"forge-queue-completion-{uuid4().hex}",
                execution_lease_seconds=30,
            )
            # The merged pull was identity-checked immediately before this fence.
            # Persist its canonical read-only receipt without a second remote read.
            if merged_intent.is_new:
                await work.operations.complete(
                    merged_intent.id,
                    self._controller.outcome(record, approved, pull),
                    owner_id=merged_intent.execution_owner,
                )
            elif merged_intent.status is not OperationStatus.SUCCEEDED:
                raise CommandRecoveryRequired("existing queue merge completion requires recovery")
            await work.releases.record_merge(run.id, pull, merged_intent.id)
            latest = await work.runs.transition(
                ...
                "run.merge_completed",
                {
                    ...
                    "merge_intent_id": str(merged_intent.id),
        ...
        if reason is not None:
            ...
            latest = await work.runs.intervene(
                run.id,
                run.version,
                "run.merge_queue_intervention",
                {
                    "observation_command_id": str(command.id),
                    "enqueue_intent_id": str(enqueue_id),
                    "reason": reason,
                },
                actor_class="worker",
                actor_id=command.actor_id,
            ...
            queued = await work.commands.enqueue(
                ...
                actor_id=command.actor_id,
                available_at=min(
                    self._clock.now() + timedelta(seconds=min(120, 15 * 2 ** min(poll, 3))),
                    deadline,
                ),
            )
            await work.events.append(
                RunEvent(
                    ...
                    event_type="run.merge_queue_observed",
                    actor_class="worker",
                    actor_id=command.actor_id,
                    occurred_at=self._clock.now(),
        ...
        await work.events.append(
            RunEvent(
                run_id=run.id,
                run_version=latest.version,
                event_type="run.merge_queue_poll_settled",
                actor_class="worker",
                actor_id=command.actor_id,
                occurred_at=self._clock.now(),
                payload={
