...
class QueueAdmissionService:
    ...
    async def execute(self, command: CommandEnvelope, work: UnitOfWork) -> None:
        ...
        interventions = [
            ...
            for event in events
            if event.event_type
            in {"run.merge_queue_admission_rejected", "run.merge_queue_admission_uncertain"}
        ...
        adapter = EnqueueOperation(
            self._controller, self._queue, record, approval_id, approved, current
        )
        ...
        if completions:
            ...
            if (
                ...
                or completions[0].run_version != run.version
                or completions[0].actor_class != "worker"
                or completions[0].actor_id != command.actor_id
        ...
        if interventions:
            ...
            if (
                ...
                or (
                    ...
                    and (
                        existing.status
                        not in {OperationStatus.NEEDS_RECONCILIATION, OperationStatus.SUCCEEDED}
                ...
                or event.payload
                != _intervention_payload(command, approval, existing, uncertain=uncertain)
            ):
        ...
        intent = await work.operations.begin(
            ...
            execution_owner=f"forge-enqueue-{uuid4().hex}",
        ...
        if merged is not None:
            ...
            completion = ObservedMergeOperation(
                self._controller, record, approval_id, approved, current
            )
            final_request = completion.request
            final_intent = await work.operations.begin(
                run_id=run.id,
                operation_type=final_request.kind,
                idempotency_key=final_request.idempotency_key,
                request_digest=final_request.request_digest,
                request_payload=final_request.request_payload,
                execution_owner=f"forge-resolved-queue-{uuid4().hex}",
                execution_lease_seconds=30,
            )
            ...
            if final_intent.is_new:
                await work.operations.complete(
                    final_intent.id,
                    self._controller.outcome(record, approved, merged),
            ...
            await work.runs.transition(
                ...
                "run.merge_completed",
                {
                    ...
                    "approval_id": str(approval_id),
                    "pull_request_id": str(record.id),
                    "merge_intent_id": str(final_intent.id),
        ...
        payload = {
            ...
            "approval_id": str(approval_id),
            "enqueue_intent_id": str(intent.id),
            "receipt_digest": canonical_digest(receipt.model_dump()),
        ...
        queued = await work.commands.enqueue(
            run_id=run.id,
            command_type="observe_merge_queue",
            idempotency_key=key,
            payload=payload,
        ...
        await work.events.append(
            RunEvent(
                ...
                event_type="run.merge_queue_enqueued",
                actor_class="worker",
                actor_id=command.actor_id,
                occurred_at=self._clock.now(),
                payload={
    ...
    async def _settle_intervention(
        ...
        *,
        ...
        matches = (
            ...
            if uncertain
            else _conclusive_rejection(intent)
        )
        if (
            not matches
        ...
        await work.runs.intervene(
            run.id,
            run.version,
            "run.merge_queue_admission_uncertain"
            if uncertain
