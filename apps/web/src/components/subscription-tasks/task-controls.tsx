...
export function TaskControls({ runId, runVersion, runAllowsExecution, runIsTerminal = false, task, onRefresh }: {
  runId: string; runVersion: number; runAllowsExecution: boolean; runIsTerminal?: boolean; task: Task; onRefresh: () => void;
}) {
  ...
  async function submit(action?: Action) {
    if (busy.current || !lifecycle.current || (!binding && (!action || !actions.includes(action) || !validReason))) return;
    ...
    try {
      ...
      if (!signal.aborted) { setBinding(null); setReason(''); setMessage(messages[result.status]); onRefresh(); }
    } catch (error) {
      if (signal.aborted) return;
      if (error instanceof ApiError && error.status === 409) {
        setBinding(null); setReason('');
        setMessage('The task or run changed. Review the refreshed state before acting again.'); onRefresh();
      ...
      } else setMessage('The request could not be confirmed. Retry the same request to retrieve its durable receipt.');
  ...
  return <section aria-label="Task controls" className="space-y-2 border-t border-slate-300 pt-3">
    ...
    {actions.length > 0 || binding ? <>
      ...
      <textarea id={reasonId} rows={2} maxLength={512} value={reason} readOnly={!!binding}
      ...
      <div className="flex flex-wrap gap-3">{binding ?
        <button type="button" disabled={pending} onClick={() => void submit()}>Retry same request</button> : <>
          {actions.includes('pause') ? <button type="button" disabled={pending || !validReason} onClick={() => void submit('pause')}>Pause task</button> : null}
          {actions.includes('resume') ? <button type="button" disabled={pending || !validReason} onClick={() => void submit('resume')}>Resume task</button> : null}
          {actions.includes('cancel') ? <button type="button" disabled={pending || !validReason} onClick={() => void submit('cancel')}>Cancel task</button> : null}
        </>}
      </div>
    </> : null}
