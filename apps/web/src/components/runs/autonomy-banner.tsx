...
import { remoteRepairRecorded } from './run-presentation';
...
export function AutonomyBanner({ projection }: { projection: components['schemas']['RunProjection'] }) {
  ...
  if (state === 'CREATED' || state === 'PLANNING') message = 'Forge may prepare a plan. Implementation requires plan approval.';
    ...
    message = remoteRepairRecorded(projection)
      ? `Forge may repair the pull request with ${projection.budgets.remote_remediation_remaining} remote repair attempts remaining. Publishing or merging still requires exact human approval.`
      : `Forge may repair local findings within the remaining ${projection.budgets.local_remediation_remaining} remediation attempts.`;
  }
